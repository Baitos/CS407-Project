    ...
    SDL_Event event { 0 };
        while (SDL_PollEvent(&event)) {
            switch (event.type) {
                ...
                {
                    handleKeyInput(state, gd, event, true, deltaTime);
                    
                    break;
                }
                case SDL_EVENT_KEY_UP:
                {   
                    handleKeyInput(state, gd, event, false, deltaTime);
                    break;
                }
                case SDL_EVENT_MOUSE_BUTTON_DOWN:
                {
                        ...
                        handleLevelClick(state, gd, gd.players_[gd.playerIndex], deltaTime, event, true);
                ...
                {
                    //for (Player &p : gd.players_) {
                        handleLevelClick(state, gd, gd.players_[gd.playerIndex], deltaTime, event, false);
                    //}
                    break;
                } 
            }
...
void handleKeyInput(const SDLState &state, GameData &gd,
    ...
    // gd.players_[gd.playerIndex].handleInput(state, gd, event, deltaTime); 
