...
void Player::collisionResponse(const SDLState &state, GameData &gd,
...
void Player::checkCollision(const SDLState &state, GameData &gd,
 	float deltaTime)
{
	SDL_FRect rectA {
	...
	for (auto &o : closeTiles_) { 
		...
		if (intersectAABB(rectA, rectB, resolution)) {
			this->collisionResponse(state, gd, (*o), rectA, rectB, resolution, deltaTime);
		}
		if (o->type == LEVEL) {
...
void Hook::checkCollision(const SDLState &state, GameData &gd, Player &p, float deltaTime) {
    if (!(this->visible)) {
        return;
    }
	SDL_FRect rectA {
    ...
    for (Player &p2 : gd.players_) {
        if (&p != &p2) { // do not check on self
            ...
            if (intersectAABB(rectA, rectB, resolution) && !this->collided) {
                ...
                p2.handleState(stunState, gd); // stun player you hit and disable their hook
            }
            Hook h2 = p2.hook;
            rectB = {
...
void Bomb::checkCollision(const SDLState &state, GameData &gd, Player &p, float deltaTime) {
    SDL_FRect rectA{
		.x = this->pos.x + this->collider.x,
		.y = this->pos.y + this->collider.y,
		.w = this->collider.w,
    ...
    for (Player &p2 : gd.players_) {
        ...
        if (intersectAABB(rectA, rectB, resolution) && this->visible) {
            ...
            p2.handleState(stunState, gd); // stun player you hit
            if (this->exploded) {
                break;
            }
            this->exploded = true;
...
void Boombox::checkCollision(const SDLState &state, GameData &gd, Player &p, float deltaTime) {
     SDL_FRect rectA{
		.x = this->pos.x + this->collider.x,
		.y = this->pos.y + this->collider.y,
		.w = this->collider.w,
...
void Ice::checkCollision(const SDLState &state, GameData &gd, Player &p, float deltaTime) {
    SDL_FRect rectA {
		.x = this->pos.x + this->collider.x,
		.y = this->pos.y + this->collider.y,
		.w = this->collider.w,
...
void Ball::checkCollision(const SDLState &state, GameData &gd, Player &p, float deltaTime) {
    SDL_FRect rectA{
		.x = this->pos.x + this->collider.x,
		.y = this->pos.y + this->collider.y,
		.w = this->collider.w,
